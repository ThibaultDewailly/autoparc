...
function getStatusLabel(isActive: boolean): string {
  return isActive ? FRENCH_LABELS.active : FRENCH_LABELS.inactive
}

export function EmployeeTable({ employees, onDelete, isLoading }: EmployeeTableProps) {
  ...
  return (
    <Table aria-label="Table des employés">
      <TableHeader>
        <TableColumn>{FRENCH_LABELS.lastName}</TableColumn>
        <TableColumn>{FRENCH_LABELS.firstName}</TableColumn>
        <TableColumn>{FRENCH_LABELS.email}</TableColumn>
        <TableColumn>{FRENCH_LABELS.role}</TableColumn>
        <TableColumn>{FRENCH_LABELS.status}</TableColumn>
        <TableColumn>{FRENCH_LABELS.lastLogin}</TableColumn>
        <TableColumn>Actions</TableColumn>
      </TableHeader>
      <TableBody
        items={employees}
        isLoading={isLoading}
        emptyContent={FRENCH_LABELS.noEmployeesFound}
      >
        {(employee) => (
          <TableRow key={employee.id}>
            ...
            <TableCell>
              <Chip color={getStatusColor(employee.isActive)} size="sm" variant="flat">
                {getStatusLabel(employee.isActive)}
              </Chip>
            </TableCell>
            <TableCell>
              {employee.lastLoginAt ? formatDate(employee.lastLoginAt) : '-'}
            </TableCell>
            <TableCell>
